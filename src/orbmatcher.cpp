...
ORBmatcher::SearchByProjection(std::shared_ptr<OrbFrame> &F, const std::vector<std::shared_ptr<OrbMapPoint>> &pMP, const float th)
{
    ...
    {
        std::shared_ptr<OrbMapPoint> mapPoint = pMP[iMP];
        if (!mapPoint->GetTrackInView())
            continue;

        if (mapPoint->IsCorrupt())
            continue;

        const int &nPredictedLevel = mapPoint->GetTrackScaleLevel();

        // The size of the window will depend on the viewing direction
        float r = RadiusByViewingCos(mapPoint->GTrackViewCos());
        ...
        const std::vector<size_t> vIndices =
                F->GetFeaturesInArea(mapPoint->getTrackProjX(), mapPoint->getTrackProjY(),
                                    r * F->m_scaleFactors[nPredictedLevel],
                                    nPredictedLevel - 1, nPredictedLevel);

        if (vIndices.empty())
            continue;

        const cv::Mat pointDescriptor = mapPoint->GetDescriptor();
        ...
        for (std::vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++)
        {
            ...
            if (F->m_mapPoints[idx])
                if (F->m_mapPoints[idx]->GetObservingKeyFrameCount() > 0)
                    continue;

            if (F->mvuRight[idx] > 0)
            {
                const float er = static_cast<const float>(fabs(mapPoint->getTrackProjX() - F->mvuRight[idx]));
                if (er > r * F->m_scaleFactors[nPredictedLevel])
                    continue;
            }

            const cv::Mat &frameDescriptor = F->m_descriptors.row(idx);
            ...
            if (dist < bestDist)
            {
                ...
                bestLevel = F->m_undistortedKeys[idx].octave;
                bestIdx = idx;
            } else if (dist < bestDist2)
            {
                bestLevel2 = F->m_undistortedKeys[idx].octave;
                bestDist2 = dist;
            }
        }
        ...
        if (bestDist <= TH_HIGH)
        {
            ...
            F->m_mapPoints[bestIdx] = mapPoint; //This needs to be done
            nmatches++;
        }
    }
...
bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint &keyPoint1, const cv::KeyPoint &keyPoint2,
                                       ...
                                       const std::shared_ptr<OrbKeyFrame> keyFrame2)
{
    ...
    const float b = keyPoint1.pt.x * fundamental.at<float>(0, 1) + keyPoint1.pt.y * fundamental.at<float>(1, 1) +
                    fundamental.at<float>(2, 1);
    const float c = keyPoint1.pt.x * fundamental.at<float>(0, 2) + keyPoint1.pt.y * fundamental.at<float>(1, 2) +
                    fundamental.at<float>(2, 2);
                            ...
                            std::vector<std::shared_ptr<OrbMapPoint>> &vpMapPointMatches)
{
    ...
    {
        if (keyFrameIt->first == featureIt->first)
        {
            const std::vector<unsigned int> vIndicesKF = keyFrameIt->second;
            const std::vector<unsigned int> vIndicesF = featureIt->second;

            for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++)
            {
                const unsigned int realIdxKF = vIndicesKF[iKF];

                std::shared_ptr<OrbMapPoint> mapPoint = mapPointsKeyFrame[realIdxKF];
                ...
                const cv::Mat &dKF = keyFrame->mDescriptors.row(realIdxKF);
                ...
                if (bestDist1 <= TH_LOW)
                {
                    ...
                    {
                        vpMapPointMatches[bestIdxF] = mapPoint;

                        const cv::KeyPoint &kp = keyFrame->mvKeysUn[realIdxKF];

                        if (mbCheckOrientation)
                        {
                            float rot = kp.angle - F->m_keys[bestIdxF].angle;
                            if (rot < 0.0)
                                rot += 360.0f;
            ...
            keyFrameIt++;
            featureIt++;
        } else if (keyFrameIt->first < featureIt->first)
        {
    ...
    if (mbCheckOrientation)
    {
        ...
        for (int i = 0; i < HISTO_LENGTH; i++)
        {
            ...
            {
                vpMapPointMatches[rotHist[i][j]] = static_cast<std::shared_ptr<OrbMapPoint>>(NULL);
                nmatches--;
            }
        }
    }
...
int ORBmatcher::SearchByProjection(std::shared_ptr<OrbKeyFrame> pKF, cv::Mat Scw,
                                   const std::vector<std::shared_ptr<OrbMapPoint>> &vpPoints,
                                   std::vector<std::shared_ptr<OrbMapPoint>> &vpMatched, int th)
{
    ...
    const float scw = static_cast<const float>(sqrt(sRcw.row(0).dot(sRcw.row(0))));
    cv::Mat Rcw = sRcw / scw;
    cv::Mat tcw = Scw.rowRange(0, 3).col(3) / scw;
    cv::Mat Ow = -Rcw.t() * tcw;

    // Set of MapPoints already found in the KeyFrame
    std::set<std::shared_ptr<OrbMapPoint>> spAlreadyFound(vpMatched.begin(), vpMatched.end());
    spAlreadyFound.erase(static_cast<std::shared_ptr<OrbMapPoint>>(NULL));
    ...
    {
        std::shared_ptr<OrbMapPoint> pMP = vpPoints[iMP];

        // Discard Bad MapPoints and already found
        if (pMP->IsCorrupt() || spAlreadyFound.count(pMP))
            continue;
        ...
        cv::Mat PO = p3Dw - Ow;
        float dist = static_cast<const float>(cv::norm(PO));

        if (dist < minDistance || dist > maxDistance)
            continue;

        // Viewing angle must be less than 60 deg
        cv::Mat Pn = pMP->GetMeanViewingDirection();
...
{
    int nmatches = 0;
    vnMatches12 = std::vector<int>(F1->m_undistortedKeys.size(), -1);
    ...
    std::vector<int> vMatchedDistance(F2->m_undistortedKeys.size(), INT_MAX);
    std::vector<int> vnMatches21(F2->m_undistortedKeys.size(), -1);
    //std::cout << "F1 size: " << F1->mvKeysUn.size() << "F2 size: " << F2->mvKeysUn.size() << std::endl;
    
    for (size_t i1 = 0, iend1 = F1->m_undistortedKeys.size(); i1 < iend1; i1++)
    {
        cv::KeyPoint kp1 = F1->m_undistortedKeys[i1];
        ...
        cv::Mat d1 = F1->m_descriptors.row(static_cast<int>(i1));
        ...
        for (std::vector<size_t>::iterator vit = vIndices2.begin(); vit != vIndices2.end(); vit++)
        {
            ...
            cv::Mat d2 = F2->m_descriptors.row(static_cast<int>(i2));
        ...
        if (bestDist <= TH_LOW)
        {
            if (bestDist < (float) bestDist2 * mfNNratio)
            {
                ...
                if (mbCheckOrientation)
                {
                    float rot = F1->m_undistortedKeys[i1].angle - F2->m_undistortedKeys[bestIdx2].angle;
                    if (rot < 0.0)
                        rot += 360.0f;
                    int bin = static_cast<int>(round(rot * factor));
                    if (bin == HISTO_LENGTH)
    ...
    for (size_t i1 = 0, iend1 = vnMatches12.size(); i1 < iend1; i1++)
        if (vnMatches12[i1] >= 0)
            vbPrevMatched[i1] = F2->m_undistortedKeys[vnMatches12[i1]].pt;

    return nmatches;
}

int ORBmatcher::SearchByBoW(std::shared_ptr<OrbKeyFrame> pKF1, std::shared_ptr<OrbKeyFrame> pKF2,
                            std::vector<std::shared_ptr<OrbMapPoint>> &vpMatches12)
{
    ...
    vpMatches12 = std::vector<std::shared_ptr<OrbMapPoint>>(vpMapPoints1.size(),
                                                            static_cast<std::shared_ptr<OrbMapPoint>>(NULL));
    ...
    OrbFeatureVector::const_iterator f1it = vFeatVec1.begin();
    OrbFeatureVector::const_iterator f2it = vFeatVec2.begin();
    OrbFeatureVector::const_iterator f1end = vFeatVec1.end();
    OrbFeatureVector::const_iterator f2end = vFeatVec2.end();
    ...
    while (f1it != f1end && f2it != f2end)
    {
        if (f1it->first == f2it->first)
        {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++)
            {
                ...
                std::shared_ptr<OrbMapPoint> pMP1 = vpMapPoints1[idx1];
                if (!pMP1)
                    continue;
                if (pMP1->IsCorrupt())
                    continue;
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++)
                {
                    ...
                    std::shared_ptr<OrbMapPoint> pMP2 = vpMapPoints2[idx2];
    ...
    if (mbCheckOrientation)
    {
        ...
        for (int i = 0; i < HISTO_LENGTH; i++)
        {
            ...
            for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++)
            {
                vpMatches12[rotHist[i][j]] = static_cast<std::shared_ptr<OrbMapPoint>>(NULL);
                nmatches--;
            }
        }
    }
...
int ORBmatcher::SearchForTriangulation(std::shared_ptr<OrbKeyFrame> pKF1, std::shared_ptr<OrbKeyFrame> pKF2,
                                       cv::Mat F12,
                                       ...
                                       const bool bOnlyStereo)
{
    ...
    int nmatches = 0;
    std::vector<bool> vbMatched2((unsigned long) pKF2->N, false);
    ...
    while (f1it != f1end && f2it != f2end)
    {
        if (f1it->first == f2it->first)
        {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++)
            {
                ...
                std::shared_ptr<OrbMapPoint> pMP1 = pKF1->GetMapPoint(idx1);
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++)
                {
                    ...
                    std::shared_ptr<OrbMapPoint> pMP2 = pKF2->GetMapPoint(idx2);
...
int ORBmatcher::Fuse(std::shared_ptr<OrbKeyFrame> pKF, const std::vector<std::shared_ptr<OrbMapPoint>> &vpMapPoints,
                     const float th)
{
    ...
    for (int i = 0; i < nMPs; i++)
    {
        ...
        if (pMP->IsCorrupt() || pMP->KeyFrameInObservingKeyFrames(pKF))
            continue;
        ...
        const float minDistance = pMP->GetMinDistanceInvariance();
        cv::Mat PO = p3Dw - Ow;
        const float dist3D = static_cast<const float>(cv::norm(PO));
        ...
        if (bestDist <= TH_LOW)
        {
            std::shared_ptr<OrbMapPoint> pMPinKF = pKF->GetMapPoint(bestIdx);
            if (pMPinKF)
            {
                if (!pMPinKF->IsCorrupt())
                {
                    if (pMPinKF->GetObservingKeyframes() > pMP->GetObservingKeyframes())
                        pMP->Replace(pMPinKF);
                    else
                        pMPinKF->Replace(pMP);
                }
            ...
            {
                pMP->AddObservingKeyframe(pKF, bestIdx);
...
int ORBmatcher::Fuse(std::shared_ptr<OrbKeyFrame> pKF, cv::Mat Scw,
                     const std::vector<std::shared_ptr<OrbMapPoint>> &vpPoints, float th,
                     std::vector<std::shared_ptr<OrbMapPoint>> &vpReplacePoint)
{
    // Get Calibration Parameters for later projection
    const float &fx = pKF->fx;
    const float &fy = pKF->fy;
    ...
    // Set of MapPoints already found in the KeyFrame
    const std::set<std::shared_ptr<OrbMapPoint>> spAlreadyFound = pKF->GetMapPoints();
    ...
    for (int iMP = 0; iMP < nPoints; iMP++)
    {
        ...
        // Project into Image
        const float invz = static_cast<const float>(1.0 / p3Dc.at<float>(2));
        const float x = p3Dc.at<float>(0) * invz;
        const float y = p3Dc.at<float>(1) * invz;
...
int ORBmatcher::SearchBySim3(std::shared_ptr<OrbKeyFrame> pKF1, std::shared_ptr<OrbKeyFrame> pKF2,
                             std::vector<std::shared_ptr<OrbMapPoint>> &vpMatches12,
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    for (int i = 0; i < N1; i++)
    {
        std::shared_ptr<OrbMapPoint> pMP = vpMatches12[i];
        if (pMP)
        {
            vbAlreadyMatched1[i] = true;
            int idx2 = pMP->GetObeservationIndexOfKeyFrame(pKF2);
            if (idx2 >= 0 && idx2 < N2)
    ...
    {
        std::shared_ptr<OrbMapPoint> pMP = vpMapPoints1[i1];
        ...
        const float invz = static_cast<const float>(1.0 / p3Dc2.at<float>(2));
        ...
        const float maxDistance = pMP->GetMaxDistanceInvariance();
        const float minDistance = pMP->GetMinDistanceInvariance();
        const float dist3D = static_cast<const float>(cv::norm(p3Dc2));
    ...
    {
        std::shared_ptr<OrbMapPoint> pMP = vpMapPoints2[i2];
        ...
        const float invz = static_cast<const float>(1.0 / p3Dc1.at<float>(2));
        ...
        const float maxDistance = pMP->GetMaxDistanceInvariance();
        const float minDistance = pMP->GetMinDistanceInvariance();
        const float dist3D = static_cast<const float>(cv::norm(p3Dc1));
...
int ORBmatcher::SearchByProjection(std::shared_ptr<OrbFrame> &CurrentFrame, const std::shared_ptr<OrbFrame> &LastFrame, const float th, const bool bMono)
{
    ...
    {
        std::shared_ptr<OrbMapPoint> pMP = LastFrame->m_mapPoints[i];

        if (pMP)
        {
            if (!LastFrame->m_outliers[i])
            {
                // Project
                ...
                int nLastOctave = LastFrame->m_keys[i].octave;

                // Search in a window. Size depends on scale
                float radius = th * CurrentFrame->m_scaleFactors[nLastOctave];
                ...
                {
                    const size_t i2 = *vit;
                    if (CurrentFrame->m_mapPoints[i2])
                        if (CurrentFrame->m_mapPoints[i2]->GetObservingKeyFrameCount() > 0)
                            continue;
                ...
                if (bestDist <= TH_HIGH)
                {
                    CurrentFrame->m_mapPoints[bestIdx2] = pMP;
                    nmatches++;

                    if (mbCheckOrientation)
                    {
                        float rot = LastFrame->m_undistortedKeys[i].angle - CurrentFrame->m_undistortedKeys[bestIdx2].angle;
                        if (rot < 0.0)
                            rot += 360.0f;
                        int bin = static_cast<int>(round(rot * factor));
                        if (bin == HISTO_LENGTH)
    ...
    if (mbCheckOrientation)
    {
        ...
        for (int i = 0; i < HISTO_LENGTH; i++)
        {
            if (i != ind1 && i != ind2 && i != ind3)
            {
                for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++)
                {
                    CurrentFrame->m_mapPoints[rotHist[i][j]] = static_cast<std::shared_ptr<OrbMapPoint>>(NULL);
                    nmatches--;
                }
            }
        }
                                   ...
                                   const int ORBdist)
{
    ...
    for (size_t i = 0, iend = vpMPs.size(); i < iend; i++)
    {
        ...
        if (pMP)
        {
            if (!pMP->IsCorrupt() && !sAlreadyFound.count(pMP))
            {
                //Project
                cv::Mat x3Dw = pMP->GetWorldPosition();
                cv::Mat x3Dc = Rcw * x3Dw + tcw;
                ...
                // Compute predicted scale level
                cv::Mat PO = x3Dw - Ow;
                float dist3D = static_cast<float>(cv::norm(PO));
                ...
                // Search in a window
                const float radius = th * CurrentFrame->m_scaleFactors[nPredictedLevel];
                ...
                if (bestDist <= ORBdist)
                {
                    ...
                    if (mbCheckOrientation)
                    {
                        float rot = pKF->mvKeysUn[i].angle - CurrentFrame->m_undistortedKeys[bestIdx2].angle;
                        if (rot < 0.0)
                            rot += 360.0f;
                        int bin = static_cast<int>(round(rot * factor));
                        if (bin == HISTO_LENGTH)
    ...
    if (mbCheckOrientation)
    {
        ...
        for (int i = 0; i < HISTO_LENGTH; i++)
        {
            if (i != ind1 && i != ind2 && i != ind3)
            {
                for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++)
                {
                    CurrentFrame->m_mapPoints[rotHist[i][j]] = NULL;
                    nmatches--;
                }
            }
        }
...
//Pointers here what to do with them
